import React from 'react';
import { Card, Col,  Pagination, Row, CardGroup, Container, Button } from 'react-bootstrap';
import './TasksDone.css';

const TasksDone = ({ tasksDoneList, currentTasksDonePage, itemsPerPage, setCurrentTasksDonePage, handleTasksDonePageChange, donePages }) => {

    return (
        <Card className='card-margin outer-card'>
        <Card.Header className='align-together'>
            <h1>TASKS DONE</h1>
        </Card.Header>
        <CardGroup className='card-group-padding'>
            <Container>
                <Row className='row-css'>
                    {tasksDoneList?.length > 0 ? tasksDoneList.slice((currentTasksDonePage - 1) * itemsPerPage, currentTasksDonePage * itemsPerPage).slice(0, 4).map((todo, index) => (
                        <Col key={todo._id}>
                            <Card className='card-container'>
                                <Card.Body>
                                    <Card.Title>{todo.text}</Card.Title>
                                    <Card.Text>
                                    {todo.description.length > 100 ?
                                        <div>
                                            {`${todo.description.substring(0, 100)}...`}
                                            <button className='button-change'>Expand More</button>
                                        </div>
                                        : 
                                        <div className='description'>
                                            {todo.description}
                                        </div>
                                    }
                                    </Card.Text>
                                    <Card.Text>
                                        <div className='due-date'>
                                            {todo.dueDate.slice(0,10)}
                                        </div>
                                    </Card.Text>
                                </Card.Body>
                                <Card.Footer>
                                    <div className='padding-buttons'>
                                        <Button size="small">DELETE</Button>
                                    </div>
                                </Card.Footer>
                            </Card>
                        </Col>
                    )) : (
                        <div class="container">
                            <p class="center-text text">No Done Tasks Yet. Good Job C:</p>
                        </div>
                    )}
                </Row>
            </Container>
        </CardGroup>

        <Pagination className='paignation-padding' onChange={handleTasksDonePageChange}>
            {donePages.map((page, index) => (
                <Pagination.Item
                    key={index}
                    active={currentTasksDonePage === page}
                    onClick={() => setCurrentTasksDonePage(page)}
                >
                    {page}
                </Pagination.Item>
        ))}
        </Pagination>

        </Card>
    );
}


export default TasksDone